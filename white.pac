...
function FindProxyForURL(o,c) {
    if(/\d+\.\d+\.\d+\.\d+/.test(c)) {
        ...
        for(var m=ip2decimal(c),left=0,right=cidrs.length;left<right;){
            var mid=Math.floor(left+(right-left)/2);
            if(m>=cidrs[mid][0]&&m<=cidrs[mid][1]){
                if("white"==mode)return"DIRECT";
                if("black"==mode)return proxy
            } else {
                m<cidrs[mid][0]?right=mid:left=mid+1
            }
        }
    }
    
    if(isPlainHostName(c)){
        return"DIRECT";
    }
